...
import {textForSpan, childByName, childrenByName, RuleTree, extractRuleTree} from "../../parserlib/ruleTree";
...
function extractCreateTableStmt(input: string, node: RuleTree): SQLCreateTableStmt {
  return {
    type: "CreateTableStmt",
    text: textForSpan(input, node.span),
    span: node.span,
    ...
    colSpec: childrenByName(node, "colSpec").map(child => extractColSpec(input, child)),
    commaWS: childrenByName(node, "commaWS").map(child => extractCommaWS(input, child))
  };
}
...
function extractMain(input: string, node: RuleTree): SQLMain {
  return {
    ...
    text: textForSpan(input, node.span),
    span: node.span,
    statement: childrenByName(node, "statement").map(child => extractStatement(input, child))
  };
}
...
function extractSelection(input: string, node: RuleTree): SQLSelection {
  return {
    ...
    text: textForSpan(input, node.span),
    span: node.span,
    columnName: childrenByName(node, "columnName").map(child => extractColumnName(input, child)),
    commaWS: childrenByName(node, "commaWS").map(child => extractCommaWS(input, child))
  };
}
function extractStatement(input: string, node: RuleTree): SQLStatement {
  return {
    ...
    createTableStmt: extractCreateTableStmt(input, childByName(node, "createTableStmt"))
  };
}
