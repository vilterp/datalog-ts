...
__commonjs = {
  0(interpreter) {
    ...
    const language = P.createLanguage({
      ...
      insert: (r) => r.record.skip(r.period).map((rec2) => ({
        type: "Insert",
        record: rec2
      })),
      ...
      ruleOptions: (r) => P.sepBy(r.andClauses, r.or).map((xs) => ({
        type: "Or",
        opts: xs
      })),
      andClauses: (r) => P.sepBy(r.clause, r.and).map((xs) => ({
        type: "And",
        clauses: xs
      })),
      ...
      arrayLit: (r) => P.seq(r.lsquare, P.sepBy(r.term, r.comma), r.rsquare).map(([_1, items, _2]) => ({
        type: "Array",
        items
      })),
      ...
      stringLit: () => P.regexp(/"((?:\\.|.)*?)"/, 1).map(interpretEscapes).desc("string").map((s) => ({
        type: "StringLit",
        val: s
      })),
      ...
      var: () => P.regex(/([A-Z][a-zA-Z0-9_]*)/, 1).map((id) => ({
        type: "Var",
        name: id
      })),
    ...
    function repeat(n, str2) {
      return repeatArr(n, str2).join("");
    }
    ...
    function arrayEq(a, b, cmp) {
      return a.length === b.length && a.reduce((accum, el, idx) => accum && cmp(el, b[idx]), true);
    }
    ...
    function doJoin(depth, invokeLoc, db, scope, clauses) {
      if (clauses.length === 1) {
        return doEvaluate(depth + 1, [...invokeLoc, {
          type: "AndClause",
          idx: 0
        }], db, scope, clauses[0]);
      }
      const leftResults = doEvaluate(depth + 1, [...invokeLoc, {
        type: "AndClause",
        idx: 0
      }], db, scope, clauses[0]);
      const out = [];
      for (const leftRes of leftResults) {
        ...
        const rightResults = doJoin(depth, [...invokeLoc, {
          type: "AndClause",
          idx: 1
        }], db, nextScope, clauses.slice(1));
    ...
    function virtualRelations(db) {
      return [...mapObjToList(db.rules, (name) => rec("internal.Relation", {
        type: str("rule"),
        name: str(name)
      })), ...mapObjToList(db.tables, (name) => rec("internal.Relation", {
        type: str("table"),
        name: str(name)
      })), ...mapObjToList(db.virtualTables, (name) => rec("internal.Relation", {
        type: str("virtual"),
        name: str(name)
      }))];
    }
    function virtualReferences(db) {
      return flatMapObjToList(db.rules, (ruleName, rule) => flatMap(rule.defn.opts, (opt) => flatMap(opt.clauses, (clause) => clause.type === "Record" ? [rec("internal.RelationReference", {
        from: str(ruleName),
    ...
    !function(n, t) {
      ...
      return function(n) {
        ...
        return r.m = n, r.c = t, r.d = function(n2, t2, e) {
          r.o(n2, t2) || Object.defineProperty(n2, t2, {
            configurable: false,
            enumerable: true,
            get: e
          });
        ...
        function j(n2, t2) {
          ...
          return {
            offset: t2,
            line: r2.length,
            column: r2[r2.length - 1].length + 1
          };
        }
        function O(n2) {
          if (!y(n2))
        ...
        function U(n2, t2, r2, e2) {
          return {
            from: n2 - t2 > 0 ? n2 - t2 : 0,
            to: n2 + r2 > e2 ? e2 : n2 + r2
          };
        }
        function W(n2, t2) {
          ...
          if (w(n2)) {
            ...
            var g2 = n2.split(/\r\n|[\n\r\u2028\u2029]/);
            r2 = c2.column - 1, e2 = c2.line - 1, o2 = U(e2, P, q, g2.length), u2 = g2.slice(o2.from, o2.to), a2 = o2.to.toString().length;
          }
          var m2 = e2 - o2.from;
        ...
        e.all = un, e.alt = T, e.any = en, e.cr = pn, e.createLanguage = function(n2) {
          ...
          int16LE: m(2),
          int32LE: m(4),
          floatBE: h("floatBE", 4).map(function(n2) {
